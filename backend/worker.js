...
async function handleSubmitVerification(request, env, origin) {
  ...
  if (existing) {
    return json({ success: true, id: existing.id, alreadySubmitted: true, paymentVerified: existing.payment_verified === 1, status: existing.status, message: existing.payment_verified === 1 ? "Wallet already verified" : "Submission found. Continue to $0.30 USDG payment." }, 200, origin);
  }
...
async function handleVerifyPayment(request, env, origin) {
  ...
  if (!wallet || !txHash || !CONFIG.chains[String(chainId)]) return json({ error: "Invalid wallet, transaction hash, or network" }, 400, origin);
  try {
...
async function handleStats(env, origin) {
  try {
    ...
    let leaderboard = [];
    try {
      const board = await env.DB.prepare("SELECT referral_code, referred_count, total_earned FROM referral_users ORDER BY COALESCE(referred_count, 0) DESC LIMIT 20").all();
...
async function handleMe(request, env, origin) {
  ...
  return json({
    ...
    message: !verification ? "This wallet has not submitted yet." : !paymentVerified ? "Details saved. Pay $0.30 USDG to verify." : ogNumber ? ("OG #" + ogNumber + ". This wallet can mint 1 NFT.") : "Verified. In WL / random pool for remaining 9,000."
  }, 200, origin);
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/" || url.pathname === "/api/health") {
      return json({ status: "online", service: "VeyroHood API", version: "0.3.0", fee: "0.30 USDG" }, 200, origin);
    }
